      ...
      <div style={{padding:"20px",maxWidth:960,margin:"0 auto"}}>
        ...
        {activeTab==="depth" && (<div>
          {players.length===0
            ...
            :<>
              <div style={{fontFamily:"'Crimson Text',serif",fontStyle:"italic",fontSize:13,color:"#5a7a9a",marginBottom:16}}>Drag players within a column to set depth order. #1 is the starter.</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-start",overflowX:"auto",paddingBottom:8}}>
                {POSITIONS.map(pos=>{
                  const col=depth[pos]||[];
                  if(col.length===0) return(<div key={pos} className="depth-col" style={{opacity:.4}}><div className="depth-col-header">{pos}</div><div style={{fontFamily:"'Crimson Text',serif",fontStyle:"italic",fontSize:12,color:"#3a5a7a",textAlign:"center",padding:"8px 0"}}>—</div></div>);
                  if(!depthRowRefs.current[pos]) depthRowRefs.current[pos]=[];
                  return(<div key={pos} className="depth-col"><div className="depth-col-header">{pos}<span style={{fontFamily:"'Crimson Text',serif",fontStyle:"italic",fontWeight:400,fontSize:10,color:"#4a6a8a",marginLeft:4}}>{POSITION_LABELS[pos]}</span></div>
                    {col.map((pid,idx)=>{const pl=playerById(pid);if(!pl) return null;const isDragging=depthDrag?.pos===pos&&depthDrag.from===idx;const isOver=depthDrag?.pos===pos&&depthDrag.over===idx&&depthDrag.from!==idx;return(<div key={pid} ref={el=>{if(!depthRowRefs.current[pos])depthRowRefs.current[pos]=[];depthRowRefs.current[pos][idx]=el;}} className={`depth-slot${isDragging?" dragging":""}${isOver?" drag-over":""}`} draggable onDragStart={e=>onDepthDragStart(e,pos,idx)} onDragOver={e=>onDepthDragOver(e,pos,idx)} onDrop={e=>onDepthDrop(e,pos,idx)} onDragEnd={()=>setDepthDrag(null)} onTouchStart={e=>onDepthTouchStart(e,pos,idx)} onTouchMove={onDepthTouchMove} onTouchEnd={onDepthTouchEnd} style={{touchAction:"none"}}><span className="depth-rank">{idx+1}</span><span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:13}}>{pl.name}</span></div>);})}
                  </div>);
                })}
              </div>
            </>
